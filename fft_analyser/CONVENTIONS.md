...
| Velocity (RMS) | `√PS/(2πf) × 1000`, 0 below 5 Hz | mm/s RMS | machine vibration |
| Displacement (RMS) | `√PS/(2πf)² × 1e6`, 0 below 5 Hz | µm RMS | low-freq faults (imbalance, looseness) |
...
diverges toward DC, so bins below a 5 Hz high-pass cutoff
(`VELOCITY_CUTOFF_HZ` — the field value specified by the client's
vibration engineer) are zeroed. The quantity is only meaningful when the input is
